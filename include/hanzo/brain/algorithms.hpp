...
//
// We use SHA-256 (twice-truncated) as a portable stand-in for BLAKE3 in
// this C++ runtime — same shape and byte-length as the other runtimes'
// stand-ins (Go uses sha256, Rust uses sha256). The address byte layout
// (version[1] + hash[20] + checksum[4]) is identical, so addresses
// produced here parse cleanly on every other Hanzo runtime.
...
// Minimal SHA-256 (stand-in for BLAKE3; matches the byte-shape of other runtimes).
...
struct Sha256 {
    ...
    void compress(const std::uint8_t* block) {
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            std::uint32_t S0 = rotr(s[0], 2) ^ rotr(s[0], 13) ^ rotr(s[0], 22);
            std::uint32_t mj = (s[0] & s[1]) ^ (s[0] & s[2]) ^ (s[1] & s[2]);
    ...
    void update(const std::uint8_t* data, std::size_t len) {
        bits += len * 8;
        while (len > 0) {
            const std::size_t take = std::min<std::size_t>(64 - buf_len, len);
            std::memcpy(buf.data() + buf_len, data, take);
            buf_len += take; data += take; len -= take;
            if (buf_len == 64) { compress(buf.data()); buf_len = 0; }
        }
    }
